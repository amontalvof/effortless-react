...
`useDeepCompareEffect` is a hook that works very similar to a normal `useEffect` but instead of comparing the references, it compares the actual values of the objects being passed to it.
